                  ...
                  finalData.map((k, idx) => (
                    <tr key={k.id} style={{ borderBottom: '1px solid #e5e7eb', background: 'white', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#f3f4f6'} onMouseLeave={e => e.currentTarget.style.background = 'white'}>
                      ...
                      <td style={{ padding: '1rem', display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                        ...
                        {/* Tombol Kelola Absen */}
                        <button 
                          title="Kelola Absensi & Cetak"
                          onClick={() => router.push(`/admin/kegiatan/${k.id}/kelola-absen`)}
                          style={{ background: '#fdf4ff', border: '1px solid #f0abfc', padding: '0.5rem', borderRadius: '6px', cursor: 'pointer', color: '#c026d3' }}
                        >
