...
export default function RekapAbsensiPage() {
  ...
  useEffect(() => {
    ...
    setPin(pinParam);
    if (kegiatanId && sesiId && pinParam) {
      fetchData(pinParam);
    } else {
      setError('PIN akses tidak ditemukan.');
      setIsLoading(false);
    }
  }, [kegiatanId, sesiId]);
  ...
  if (error) return <div style={{ padding: '4rem', textAlign: 'center', color: 'red' }}><h2>Error</h2><p>{error}</p></div>;
      ...
      <main className="container" style={{ padding: '2rem 1rem' }}>
        ...
        <div className="no-print" style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <button onClick={() => window.location.href = `/portal-panitia/absen/${kegiatanId}?pin=${pin}`} className="btn btn-outline">← Kembali</button>
            {sesi.status === 'aktif' && (
