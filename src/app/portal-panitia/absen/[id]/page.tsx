...
export default function KelolaAbsenPage() {
  ...
  useEffect(() => {
    ...
    if (kegiatanId && pinParam) {
      fetchData(pinParam);
    } else {
      setError('PIN akses tidak ditemukan. Harap buka halaman ini melalui Portal Panitia.');
      setIsLoading(false);
    }
  }, [kegiatanId]);

  const fetchData = async (pinAkses: string) => {
    setIsLoading(true);
    
    // 1. Fetch Kegiatan
    const { data: keg, error: kegErr } = await supabase.from('kegiatan').select('*').eq('id', kegiatanId).single();
    if (kegErr || !keg) {
      setError('Kegiatan tidak ditemukan');
      setIsLoading(false);
      return;
    }
    if (keg.pin_akses !== pinAkses) {
      setError('PIN akses salah atau tidak valid');
      setIsLoading(false);
      return;
    }
    setKegiatan(keg);
  ...
  if (error) {
    return (
      <div className="container" style={{ padding: '4rem 1rem', textAlign: 'center' }}>
        <div className="card" style={{ padding: '2rem', color: '#b91c1c', borderTop: '4px solid #ef4444' }}>
          <h2>Akses Ditolak</h2>
          <p>{error}</p>
          <button onClick={() => router.push('/portal-panitia/hasil')} className="btn btn-outline" style={{ marginTop: '1rem' }}>Kembali</button>
        </div>
      </div>
    );
  }
  ...
  if (!kegiatan) return <div style={{ padding: '4rem', textAlign: 'center' }}>Kegiatan tidak ditemukan.</div>;
              ...
              {sesiList.map(sesi => (
                <div key={sesi.id} style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <Link href={`/portal-panitia/absen/${kegiatanId}/rekap?sesi_id=${sesi.id}&pin=${pin}`} className="btn btn-primary" style={{ padding: '0.5rem 1rem', fontSize: '0.85rem' }}>
                      Rekap Kehadiran
                    </Link>
