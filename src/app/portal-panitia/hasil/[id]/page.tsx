...
export default function LaporanAnalisisPanitia({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div style={{ background: 'white', minHeight: '100vh' }}>
      ...
      <div className="no-print" style={{ background: '#1e40af', color: 'white', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
          <button 
            onClick={() => router.push('/')} 
            className="btn btn-outline" 
            style={{ color: 'white', borderColor: 'rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem' }}
          >
                ...
                <div key={section.id} style={{ marginBottom: '2rem' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                    <thead style={{ display: 'table-header-group' }}>
                      <tr>
                        <th colSpan={3} style={{ padding: 0, paddingBottom: '1rem' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc', padding: '0.75rem 1rem', border: '1px solid #e2e8f0', fontWeight: 'normal' }}>
                            <h4 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 'bold' }}>{section.nama_section}</h4>
                            {sectionAverages[section.id] && (
